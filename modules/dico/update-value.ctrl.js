...
module.exports.main = (req, res, next) => {
    ...
    validator.uuid(ps.item_uuid, "ps.item_uuid")
        ...
        .then(() => {
            return miscellaneousReq.access2Note(tx, ps.uid, ps.item_uuid)
        })
        .then(() => {
            return miscellaneousReq.access2Note(tx, ps.uid, ps.up_uuid)
        })
        .then(() => {
            let query = `
      ...
      RETURN {uuid:up.uuid, value:up.value, code_label:up.code_label}`;
        ...
        .catch(err => {
            console.log(err);
            utils.fail(session, {status: err.status || 400, mess: err.mess || 'dico/update-value.ctr.js/main'}, res, tx)
        })
};
