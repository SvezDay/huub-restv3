...
module.exports.createItemAndDefintion = (tx, idx_uuid, item_code_label, item_value, def_code_label, def_value) => {
    ...
    `;
        ...
        tx.run(query, {
            idx_uuid: idx_uuid,
            item_code_label: item_code_label,
            item_value: item_value,
            def_code_label: def_code_label,
            def_value: def_value
        })
        // .then(parser.parse).then(data=>{console.log(data)})
            ...
            .catch(err => {
                console.log(err);
                reject({status: err.status || 400, mess: err.mess || '_dico/create-item.ctrl.js/createItem'});
            })
    })
};
/*
    ...
    validator.uuid(ps.idx_uuid, 'ps.idx_uuid')
        ...
        .then(() => {
            ...
            if (!languageCodes.includes(ps.code_label)) {
                throw {
                    status: 403,
                    mess: '_dico/create-item.ctrl.js/main code_label is not on the list of language label'
                };
            }
        })
        ...
        .catch(err => {
            console.log(err);
            utils.fail(session, {status: err.status || 400, mess: err.mess || 'dico/create-item.ctrl.js/main'}, res, tx)
        })
};
