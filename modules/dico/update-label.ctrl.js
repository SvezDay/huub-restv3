        ...
        tx.run(query, {up_uuid: up_uuid, code_label: code_label})
            ...
            .catch(err => {
                ...
                reject({
                    ...
                    mess: err.mess || 'dico/read-extend-head-graph.ctrl.js/getExtendHeadGraph'
                });
            })
    })
}
...
module.exports.main = (req, res, next) => { // Input:  first_uuid, up_uuid, code_label  |  Output: VocabularyGraph
    ...
    validator.uuid(ps.first_uuid, 'ps.first_uuid')
        ...
        .catch(err => {
            console.log(err);
            utils.fail(session, {status: err.status || 400, mess: err.mess || 'dico/update-label.ctr.js/main'}, res, tx)
        })
};
