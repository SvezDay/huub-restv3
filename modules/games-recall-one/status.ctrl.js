...
module.exports.main = (req, res, next) => {
    ...
    validator.uuid(ps.idx_uuid, "ps.idx_uuid")
        ...
        .catch(err => {
            console.log(err);
            utils.fail({status: err.status || 400, mess: err.mess || 'games-recall-one/status.ctrl.js/main'}, res, tx)
        })
};
...
module.exports.recallStatus = (req, res, next) => {
    ...
    validator.uuid('ps.recall.uuid', ps.recall.uuid)
      ...
      SET r.status = ${ps.status}`;
        ...
        .catch(err => {
            console.log(err);
            reject({status: err.status || 400, mess: err.mess || 'game-recall-one/status.ctrl.js/recallStatus'});
        })
};
