...
module.exports.getRecalls = (tx, uid) => {
    ...
    `;
        tx.run(quer).then(parser.parse)
            .then(res => {
                console.log("getCourseIndex ", res);
                return res
            })
            .then(recalls => resolve(recalls[0]))
            .catch(err => {
                console.log(err);
                reject({status: err.status || 400, mess: err.mess || 'game-recall-one/maj.ctrl.js/getRecalls'});
            })
    })
}
/*
...
module.exports.getCourseIndex = (tx, uid) => {
    ...
    RETURN collect(distinct is) `;
        ...
        // .then(res => {console.log("getCourseIndex ", res[0]); return res})
            ...
            .catch(err => {
                console.log(err);
                reject({status: err.status || 400, mess: err.mess || 'game-recall-one/maj.ctrl.js/getCourseIndexs'});
            })
    })
}
/*
...
module.exports.getCombination = (tx, uid, indexList) => {
    return new Promise((resolve, reject) => {
        ...
        allPromises.then(() => {
            ...
            .catch(err => {
                console.log(err);
                reject({status: err.status || 400, mess: err.mess || 'game-recall-one/maj.ctrl.js/getCombination'});
            })
    })
}
...
module.exports.main = (req, res, next) => {
    ...
    // .then(result => {console.log("getCourseIndex ", result); return result})
        ...
        .then(result => {
            console.log("getCombination ", result);
            return result
        })
        .then(result => {
            if (!!result) {
        ...
        .then(list => {
            return !!list.length ? recallReq.createRecall(tx, ps.uid, list) : null
        })

        .then(() => this.getRecall2Delete(combinationList, recallList))
        // .then(result => {console.log("getRecall2Delete ", result); return result})
        .then(list => {
            return !!list.length ? recallReq.deleteRecall(tx, list) : null
        })
        ...
        .catch(err => {
            console.log(err);
            utils.fail({status: err.status || 400, mess: err.mess || 'games-recall-one/maj.ctrl.js/main'}, res, tx)
        })
};
