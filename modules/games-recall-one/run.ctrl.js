    ...
    return new Promise((resolve, reject) => {
      ...
      AND r.deadline < ${now} `;
        ...
        // .then(recall => {if(!recall.length){throw {status: 204, mess: 'no more card'}}else{resolve(recall[0])}  })
            ...
            .catch(err => {
                console.log(err);
                reject({status: err.status || 400, mess: err.mess || 'game-recall-one/run.ctrl.js/getRecall'});
            })
    })
}
...
module.exports.getIdxDecendant = (tx, uid, idx) => {
    ...
    `;
        tx.run(query, {uid: uid, idx: idx}).then(parser.parse)
            .then(data => {
                console.log('getIdxDecendant :: data: ', data);
                return data;
            })
            .then(indexList => {
                resolve(indexList[0])
            ...
            .catch(err => {
                console.log(err);
                reject({status: err.status || 400, mess: err.mess || 'game-recall-one/run.ctrl.js/getIdxDecendant'});
            })
    })
}
...
module.exports.getQA = (tx, recall) => {
    ...
    `;
        ...
        return tx.run(query, {from: recall.from, to: recall.to}).then(parser.parse)
            ...
            .catch(err => {
                console.log(err);
                reject({status: err.status || 400, mess: err.mess || 'game-recall-one/run.ctrl.js/getQA'});
            })
    })
}
...
module.exports.main = (req, res, next) => {
    ...
    validator.uuid(ps.uid, "ps.uid")
        .then(() => {
            return ps.hasOwnProperty('idx_uuid') ? validator.uuid(ps.idx_uuid, "ps.idx_uuid") : null
        })
        ...
        .catch(err => {
            console.log(err);
            utils.fail({status: err.status || 400, mess: err.mess || 'games-recall-one/run.ctrl.js/main'}, res, tx)
        })
};
