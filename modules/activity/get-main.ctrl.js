    ...
    return new Promise((resolve, reject) => {
      ...
      RETURN {index:{uuid:i.uuid, model:i.model}, title:{uuid:t.uuid, value:t.value}} `;
        ...
        // .then(result=>{console.log('result', result); return result; })
            ...
            .catch(err => {
                console.log(err);
                reject({status: err.status || 400, mess: err.mess || 'activity/getMain.ctrl.js/getMain'});
            })
    })
}
...
module.exports.main = (req, res, next) => {
    ...
    validator.uuid(ps.uid, 'ps.uid')
        ...
        .catch(err => {
            console.log(err);
            utils.fail(session, {status: err.status || 400, mess: err.mess || 'activity/get-main.ctr.js/main'}, res, tx)
        })
