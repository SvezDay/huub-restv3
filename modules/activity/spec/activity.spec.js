...
describe('CREATE ACTIVITY',()=>{
  it('should create a first todo', (done)=>{
    chai.request('http://localhost:3200')
    .post('/modules/activity/create-activity').send({model:'todo'}).set('x-access-token',token)
    .end((err, res)=>{
      res.should.have.status(200);
      res.body.should.be.a('object');
      res.body.should.have.property('token');
  ...
  it('should return first todo', (done)=>{
    chai.request('http://localhost:3200')
    .get('/modules/activity/get-main').set('x-access-token',token)
    .end((err, res)=>{
      res.should.have.status(200);
      res.body.should.have.property('data');
  ...
  it('should create an event', (done)=>{
    chai.request('http://localhost:3200')
    .post('/modules/activity/create-event').set('x-access-token',token).send({parent_uuid:todoA.title.uuid})
    .end((err, res)=>{
      res.should.have.status(200);
