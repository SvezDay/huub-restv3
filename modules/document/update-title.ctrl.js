...
module.exports.updateTitle = (tx, uuid, value) => {
    ...
    `;
        tx.run(query, {uuid: uuid, value: value})
            ...
            .catch(err => {
                console.log(err);
                reject({status: err.status || 400, mess: err.mess || 'document/update-title.ctrl.js/updateTitle'});
            })
    })
}
/*
...
module.exports.main = (req, res, next) => {
    ...
    validator.uuid(ps.up_uuid, 'ps.up_uuid')
        .then(() => {
            return validator.uuid(ps.idx_uuid, 'ps.idx_uuid')
        })
        .then(() => {
            return validator.str(ps.value)
        })
        .then(() => {
            return miscellaneousReq.access2Index(tx, ps.uid, ps.idx_uuid)
        })
        .then(() => {
            return miscellaneousReq.access2Any(tx, ps.uid, ps.up_uuid)
        })
        .then(() => {
            return this.updateTitle(tx, ps.up_uuid, ps.value)
        })
        .then(() => {
            return graphReq.getHeadGraph(tx, ps.idx_uuid)
        })
        ...
        .catch(err => {
            ...
            utils.fail(session, {
                ...
                mess: err.mess || 'document/update-title.ctrl.js/main'
            }, res, tx)
        })
};
