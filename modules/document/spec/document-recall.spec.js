...
xdescribe('TEST DOCUMENT - RECALL FLOW #B & #D', ()=>{
  ...
  describe('CREATE DOCUMENTS PREREQUIS',()=>{
    ...
    it('should create first note', (done)=>{
      chai.request('http://localhost:3200')
      .post('/modules/document/create-note').set('x-access-token',token).send({parent_uuid:docs.title.uuid, model:'blank', code_label:5.1}) // code_label:principe
      .end((err, res)=>{
        res.should.have.status(200);
        docs.notes = [];
    ...
    it('should create second note', (done)=>{
      chai.request('http://localhost:3200')
      .post('/modules/document/create-note').set('x-access-token',token).send({parent_uuid:docs.notes[0].uuid, model:'blank', code_label:5.2}) // code_label:definition
      .end((err, res)=>{
        res.should.have.status(200);
        docs.notes.push(res.body.data);
        done();
...
xdescribe('TEST DOCUMENT - RECALL FLOW #B & #E', ()=>{
  ...
  describe('DELETE PREREQUIS AS FLOW #E', ()=>{
    it('should delete note', (done)=>{
      chai.request('http://localhost:3200')
      .delete('/modules/document/delete-note').set('x-access-token',token).set('note_uuid',docs.notes[0].uuid)
      .end((err, res)=>{
        res.should.have.status(200);
        done();
      })
...
describe('TEST DOCUMENT - RECALL WITH BIG DICO IMPORTED', function(){
  ...
  describe('IMPORT DICO',()=>{
    it('should import dico', (done)=>{
      chai.request('http://localhost:3200')
      .put('/modules/games/update-brut-data').set('x-access-token',token).send({file:'ef4.data.js'})
      .end((err, res)=>{
        res.should.have.status(200);
  ...
  describe('DELETE PREREQUIS', ()=>{
    it('should delete the document', (done)=>{
      chai.request('http://localhost:3200')
      .delete('/modules/document/delete-document').set('x-access-token',token).set('idx_uuid',docs.index.uuid) // code_label:definition
      .end((err, res)=>{
        res.should.have.status(200);
        done();
      })
