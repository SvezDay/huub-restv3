...
module.exports.updateLabel = (tx, uuid, code_label) => { //  Input: tx, uuid, code_label  |  Output: void
    return new Promise((resolve, reject) => {
        ...
        SET n.code_label = $code_label `;
            ...
            .catch(err => {
                console.log(err);
                reject({status: err.status || 400, mess: err.mess || 'document/update-note-label.ctrl.js/updatLabel'});
            })
    })
}
...
module.exports.main = (req, res, next) => { // Input: idx_uuid, up_uuid, code_label  |  Output: ExtendGraph
    ...
    validator.uuid(ps.idx_uuid, "ps.idx_uuid")
        ...
        .then(result => {
            utils.commit(session, tx, res, ps.uid, result)
        })
        .catch(err => {
            console.log(err);
            utils.fail(session, {
                status: err.status || 400,
                mess: err.mess || 'document/update-note.ctrl.js/main'
            }, res, tx)
        })
};
