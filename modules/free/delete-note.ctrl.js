...
const relationReq = require('../../services/relation.request');
// COMMON ----------------------------------------------------------------------
// CONTROLLERS -----------------------------------------------------------------
...
module.exports.leveledCollection = (tx, parent_uuid, collection) => {
    ...
    return new Promise((resolve, reject) => {
        ...
        return tx.run(leveled1 + leveled2, {parent_uuid: parent_uuid})
            ...
            .catch(err => {
                console.log(err);
                reject({status: err.status || 400, mess: err.mess || 'free/delete-note.ctrl.js/leveledCollection'});
            })
    })
}
...
module.exports.getNoteAround = (tx, note_uuid) => {
    ...
    `;
        return tx.run(collectionNote, {note_uuid: note_uuid}).then(parser.parse)
            ...
            .catch(err => {
                console.log(err);
                reject({status: err.status || 400, mess: err.mess || 'free/delete-note.ctrl.js/getNoteAround'});
            })
    })
}
...
module.exports.deleteNote = (tx, note_uuid) => {
    return new Promise((resolve, reject) => {
        ...
        return tx.run(deleteIt, {note_uuid: note_uuid}).then(parser.parse)
            ...
            .catch(err => {
                console.log(err);
                reject({status: err.status || 400, mess: err.mess || 'free/delete-note.ctrl.js/deleteNote'});
            })
    })
}
...
module.exports.main = (req, res, next) => {
    ...
    validator.uuid(ps.note_uuid)
        .then(() => {
            return validator.uuid(ps.idx_uuid)
        })
        .then(() => {
            return miscellaneousReq.access2Note(tx, ps.uid, ps.note_uuid)
        })
        .then(() => {
            return miscellaneousReq.access2Index(tx, ps.uid, ps.idx_uuid)
        })
        .then(() => {
            return descendantReq.getNoteAncestorAndDescendant(tx, ps.note_uuid)
        })
        .then(result => {
            // console.log('============================================================')
        ...
        .then(() => {
            return this.getNoteAround(tx, ps.note_uuid)
        })
        .then(result => {
        ...
        .then(() => {
            utils.commit(session, tx, res, ps.uid)
        })
        .catch(err => {
            console.log(err);
            utils.fail(session, {status: err.status || 400, mess: err.mess || 'free/delete-note.ctrl.js/main'}, res, tx)
        })
};
