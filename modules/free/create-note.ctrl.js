...
module.exports.createNote = (tx, idx_uuid) => {
    ...
    CALL apoc.do.when(count=1,
      ...
      RETURN new`
        ...
        return tx.run(query, {idx_uuid: idx_uuid})
            ...
            .catch(err => {
                console.log(err);
                reject({status: err.status || 400, mess: err.mess || 'free/create-note.ctrl.js/createNote'});
            })
    })
}
/*
...
module.exports.main = (req, res, next) => {
    ...
    validator.uuid(ps.uuid)
        .then(() => {
            return miscellaneousReq.access2Index(tx, ps.uid, ps.uuid)
        })
        ...
        .catch(err => {
            console.log(err);
            utils.fail(session, {status: err.status || 400, mess: err.mess || 'free/create-note.ctrl.js/main'}, res, tx)
        })
};
