    ...
    return new Promise((resolve, reject) => {
      ...
      RETURN EXTRACT(x IN COLLECT( DISTINCT notes) | x.uuid)`;
        tx.run(query, {idx_uuid: idx_uuid}).then(parser.parse)
            ...
            .catch(err => {
                console.log(err);
                reject({status: err.status || 400, mess: err.mess || 'free/update-order.ctrl.js/compareOrder'});
            })
    })
}
/*
    ...
    return new Promise((resolve, reject) => {
      ...
      FOREACH(r IN relationships(p2) | DELETE r)`;
        tx.run(query, {idx_uuid: idx_uuid}).then(parser.parse)
            ...
            .catch(err => {
                console.log(err);
                reject({status: err.status || 400, mess: err.mess || 'free/update-order.ctrl.js/deleteOldRelation'});
            })
    })
}
/*
...
module.exports.createRelationOrder = (tx, idx_uuid, order_list) => {
    return new Promise((resolve, reject) => {
        ...
        tx.run(qOne + qTwo, {idx_uuid: idx_uuid})
            ...
            .catch(err => {
                console.log(err);
                reject({status: err.status || 400, mess: err.mess || 'free/update-order.ctrl.js/createRelationOrder'});
            })
    })
}
/*
    ...
    validator.uuid(ps.idx_uuid)
        ...
        .catch(err => {
            ...
            utils.fail(session, {
                ...
                mess: err.mess || 'free/update-order.ctrl.js/main'
            }, res, tx)
        })
};
