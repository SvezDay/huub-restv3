...
const ancestorReq = require('../../services/ancestor.request');
...
module.exports.getTreeByIndex = (tx, uid, idx_uuid) => {
    return new Promise((resolve, reject) => {
        ...
        // .then(data=>{console.log('getancestor data ===================', data); return data;})
            ...
            .then(data => {
                console.log('getTitleDescendant data ===================', data);
                return data;
            })
            .then(result => {
                tree.titleDescendant = result;
            })
            .then(() => descendantReq.getAllNoteDescendant(tx, uid, idx_uuid))
            // .then(data=>{console.log('getAllNoteDescendant data ===================', data); return data;})
            .then(result => {
                tree.noteDescendant = result;
            })
            .then(() => console.log("tree", tree))
            .then(() => resolve(tree))
            .catch(err => {
                console.log(err);
                reject({status: err.status || 400, mess: err.mess || 'tree/tree.ctrl.js/getTreeByIndex'});
            })
    })
};
/*
...
module.exports.getTreeFromRoot = (tx, uid) => {
    return new Promise((resolve, reject) => {
        ...
        getMainCtrl.getMain(tx, uid)
            ...
            .catch(err => {
                console.log(err);
                reject({status: err.status || 400, mess: err.mess || 'tree/tree.ctrl.js/getTreeFromRoot'});
            })
    })
};
/*
...
module.exports.main = (req, res, next) => {
    ...
    validator.isUndefined(ps.idx_uuid, 'ps.idx_uuid')
        ...
        .catch(err => {
            console.log(err);
            utils.fail(session, {status: err.status || 400, mess: err.mess || 'tree/tree.ctrl.js/lose'}, res, tx)
        })
};
