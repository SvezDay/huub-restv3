...
module.exports.access2Index = (tx, uid, idx_uuid) => {
    return new Promise((resolve, reject) => {
        ...
        return tx.run(query, {uid: uid, idx_uuid: idx_uuid}).then(parser.parse)
            .then(result => {
                if (!result[0]) {
                    throw {status: 403, mess: `access2Index() user access failed: ${idx_uuid}: user:${uid}`}
                }
            })
            .then(result => resolve())
            .catch(err => {
                ...
                reject({
                    ...
                    mess: err.mess || 'services/miscellaneous.request.js/access2Index'
                });
            })
    })
}
...
module.exports.access2Title = (tx, uid, tit_uuid) => {
    return new Promise((resolve, reject) => {
        ...
        tx.run(query, {uid: uid, tit_uuid: tit_uuid}).then(parser.parse)
            .then(result => {
                if (!result[0]) {
                    throw {status: 403, mess: `access2Title() user access failed: ${tit_uuid}`}
                }
            })
            .then(result => resolve())
            .catch(err => {
                ...
                reject({
                    status: err.status || 400,
                    mess: err.mess || 'services/miscellaneous.request.js/access2Title'
                });
            })
    })
}
...
module.exports.access2Note = (tx, uid, note_uuid) => {
    return new Promise((resolve, reject) => {
        ...
        tx.run(query, {uid: uid, note_uuid: note_uuid}).then(parser.parse)
            .then(result => {
                if (!result[0]) {
                    throw {status: 403, mess: `access2Note() user access failed: ${note_uuid}`}
                }
            })
            .then(result => resolve())
            .catch(err => {
                console.log('>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>', err);
                reject({status: err.status || 400, mess: err.mess || 'services/miscellaneous.request.js/access2Note'});
            })
    })
}
...
module.exports.access2Any = (tx, uid, any_uuid) => {
    return new Promise((resolve, reject) => {
        ...
        tx.run(query, {uid: uid, any_uuid: any_uuid}).then(parser.parse)
            .then(result => {
                if (!result[0]) {
                    throw {status: 403, mess: `access2Any() user access failed: ${any_uuid}`}
                }
            })
            .then(result => resolve())
            .catch(err => {
                console.log('>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>', err);
                reject({status: err.status || 400, mess: err.mess || 'services/miscellaneous.request.js/access2Any'});
            })
    })
}
/*
...
module.exports.access2NoteOrTitle = (tx, uid, xxx_uuid) => {
    return new Promise((resolve, reject) => {
        ...
        tx.run(query, {uid: uid, xxx_uuid: xxx_uuid}).then(parser.parse)
            .then(result => {
                if (!result[0]) {
                    throw {status: 403, mess: `access2NoteOrTitle() user access failed: ${xxx_uuid}`}
                }
            })
            .then(result => resolve())
            .catch(err => {
        ...
        if (xxx_uuid == null) {
            ...
            tx.run(query, {uid: uid, xxx_uuid: xxx_uuid}).then(parser.parse)
                ...
                .catch(err => {
                    ...
                    reject({
                        status: err.status || 400,
                        mess: err.mess || 'services/miscellaneous.request.js/access2Note'
                    });
                })
        }
    })
