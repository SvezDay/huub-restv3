        ...
        tx.run(collectionIndex).then(parser.parse)
            ...
            .catch(err => {
                ...
                reject({
                    ...
                    mess: err.mess || 'services/descendant.request.js/getNoteAncestorAndDescendant'
                });
            })
    })
}
      ...
      WITH list + collect({note_uuid:n_${n.id}.uuid, index: {uuid:i_${n.id}.uuid, model:i_${n.id}.model}, title:{ uuid:t_${n.id}.uuid, value:t_${n.id}.value} }) AS list`;
        ...
        tx.run(q2one + q2two).then(parser.parse)
            ...
            .catch(err => {
                ...
                reject({
                    status: err.status || 400,
                    mess: err.mess || 'services/descendant.request.js/getGraphDescendant'
                });
            })
    })
}
    ...
    return new Promise((resolve, reject) => {
        ...
        Promise.resolve().then(() => {
            ...
            .catch(err => {
                ...
                reject({
                    status: err.status || 400,
                    mess: err.mess || 'services/descendant.request.js/bindNoteToDescendant'
                });
            })
    })
}
    ...
    `;
        ...
        // .then(data => {console.log("getAllNoteDescendant", data); return data} )
            ...
            .catch(err => {
                ...
                reject({
                    status: err.status || 400,
                    mess: err.mess || 'services/descendant.request.js/getAllNoteDescendant'
                });
            })
    })
}
      ...
      RETURN {index:{uuid:idx.uuid, model:idx.model}, title:{uuid:title.uuid, value:title.value} } `;
        ...
        // .then(data => {console.log("getTitleDescendant ============================", data); return data} )
            ...
            .catch(err => {
                ...
                reject({
                    status: err.status || 400,
                    mess: err.mess || 'services/descendant.request.js/getTitleDescendant'
                });
            })
    })
}
      ...
      RETURN {index:{uuid:idx.uuid, model:idx.model}, title:{uuid:title.uuid, value:title.value} } `;
        ...
        // .then(data => {console.log("getTitleDescendant ============================", data); return data} )
            ...
            .catch(err => {
                ...
                reject({
                    status: err.status || 400,
                    mess: err.mess || 'services/descendant.request.js/getTitleDescendantByUid'
                });
            })
    })
}
