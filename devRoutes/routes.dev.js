...
const utils = require('../modules/services/utils.service');
const validator = require('../modules/services/validator.service');
let tokenGen = require('../modules/services/token.service');
...
module.exports.clearDatabase = (req, res, next) => {
    let session = driver.session();
    let tx = session.beginTransaction();
    tx.run('MATCH (n) DETACH DELETE n').then(() => {
        tx.commit();
        session.close();
        res.status(200).json({});
    });
...
module.exports.allDocument = (req, res, next) => {
    ...
    tx.run(query).then(parser.parse)
        .then(data => {
            console.log('allDocument devRoutes: data', data);
            return data;
        })
        .then(data => {
            tx.commit();
